...
u "${tmp_hfps_nga}/panel/access.dta", clear
...
wordsearch "*ocket* *OCKET*", dir("${raw_hfps_nga1}")
	*-> rounds 13 15 16 17 18 20, sect_5 

loc sect_5 : dir "${raw_hfps_nga2}" files "p2r*_sect_5.dta"
loc f=1
foreach s5 of local sect_5 {
	u s5* using "${raw_hfps_nga2}/`s5'", clear
	la dir
...
loc f=1	
foreach s5 of local sect_5 {
	mer 1:1 lname value label using `s5`f'', gen(_`f')
	la val _`f' . 
	recode _`f' (1 .=.)(2 3=`f') 
	loc ++f 
}
...
loc f=1	
foreach s5 of local sect_5 {
	mer 1:1 name varlab using `s5`f'', gen(_`f')
	la val _`f' . 
	recode _`f' (1 .=.)(2 3=`f') 
	loc ++f 
}
...
sa "${tmp_hfps_nga}/panel/gff.dta", replace 
u  "${tmp_hfps_nga}/panel/gff.dta", clear 
ta item round
