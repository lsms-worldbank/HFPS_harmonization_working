...
mer 1:1 round hhid using "${tmp_hfps_nga}/panel/cover.dta", keepus(sector wgt)
ta round _m
...
mer 1:1 round hhid using "${tmp_hfps_nga}/panel/demog.dta", keepus(hhsize)
ta round _m
bys round (hhid) : egen min_m=min(_merge)
bys round (hhid) : egen max_m=max(_merge)
...
sa "${tmp_hfps_nga}/panel/fies.dta", replace
