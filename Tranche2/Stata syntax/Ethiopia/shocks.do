...
egen shock_cope_1	= anymatch(`sc'), v(10/15)	
egen shock_cope_22	= anymatch(`sc'), v(18)	
egen shock_cope_21	= anymatch(`sc'), v(19)	
egen shock_cope_96	= anymatch(`sc'), v(-96)	

collapse (max) yn shock_cope_*, by(household_id round shock_code)
	
order shock_cope_*, seq a(yn)
...
sa "${tmp_hfps_eth}/panel/shocks.dta", replace
	
	
*	move to household level for analysis
u  "${tmp_hfps_eth}/panel/shocks.dta", clear
...
collapse (max) shock_yn* shock_cope*, by(household_id round)
 foreach x of varlist shock_yn* shock_cope* {
 	la var `x' "`lbl`x''"
 }
...
sa "${tmp_hfps_eth}/panel/hh_shocks.dta", replace 
...
sa "${tmp_hfps_eth}/panel/hh_income_loss.dta", replace 
