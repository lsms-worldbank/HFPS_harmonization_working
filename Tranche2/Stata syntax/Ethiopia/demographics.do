...
tab2 round bi2_hhm_new bi3_hhm_stillm, first
	  ...
	  //respondent	
	  d using "${tmp_hfps_eth}/panel/p1_cover.dta"
	  g ii4_resp_id = individual_id

	 /*	old investigations
	mer 1:1 household_id ii4_resp_id round using "${tmp_hfps_eth}/panel/p1_cover.dta"
	la drop _merge
	la val _merge
...
// 	li round individual_id ii4_resp_id _me if household_id=="120203010100102031", sepby(round)	//->	now addressed in fix in cover.do
	...
	*/
	mer 1:1 household_id ii4_resp_id round using "${tmp_hfps_eth}/panel/p1_cover.dta", assert(1 3) keepus(ii4_resp_id ii4_resp_gender ii4_resp_age ii4_resp_relhhh)
		gen respond=(_m==3) 
	la drop _merge
	drop _merge
...
** DROP UNNECESSARY VARIABLES		 
	  ...
	  sa "${tmp_hfps_eth}/panel/p1_ind.dta", replace
...
replace round=round+12	//	consistency with phase 1 
	...
	isid household_id individual_id round
	  ...
	  //respondent	
	  d using "${tmp_hfps_eth}/panel/p2_cover.dta"
	  g ii4_resp_id = individual_id
	  
	mer 1:1 household_id ii4_resp_id round using "${tmp_hfps_eth}/panel/p2_cover.dta", keepus(ii4_resp_id ii4_resp_same ii4_resp_gender ii4_resp_age ii4_resp_relhhh)
...
** DROP UNNECESSARY VARIABLES		 
	  ...
	  isid household_id individual_id round
	  sort household_id individual_id round
	  sa "${tmp_hfps_eth}/panel/p2_ind.dta", replace

	  
*	combine individual datasets
clear 
append using "${tmp_hfps_eth}/panel/p1_ind.dta" "${tmp_hfps_eth}/panel/p2_ind.dta", gen(phase)
la drop _append
la val phase .
la val relation relation
	...
	*	make some choices
	...
	u household_id round ii4_* if inlist(household_id,"`i'") using "${tmp_hfps_eth}/panel/p2_cover.dta", clear
	li, sep(0)
	}
...
isid household_id individual_id round
sort household_id individual_id round
sa "${tmp_hfps_eth}/panel/ind.dta", replace
...
sa "${tmp_hfps_eth}/panel/demog.dta", replace
...
mer 1:1 household_id round using "${tmp_hfps_eth}/panel/demog.dta"
ta round _m
...
isid household_id individual_id round
mer 1:1  household_id individual_id round using "${tmp_hfps_eth}/panel/p1_demog.dta"
