...
sa "${tmp_hfps_eth}/panel/p1_cover.dta", replace
...
sa "${tmp_hfps_eth}/panel/p2_cover.dta", replace


u  "${tmp_hfps_eth}/panel/p2_cover.dta", clear


clear 
append using "${tmp_hfps_eth}/panel/p1_cover.dta" "${tmp_hfps_eth}/panel/p2_cover.dta", gen(phase)
isid household_id round

table (start_yr start_mo) round, nototal

*	tabulating the change in sample (r1 is p1 baseline, r13 is p2 baseline)
u if round==1 using "${tmp_hfps_eth}/panel/p1_cover.dta", clear
tempfile p1r1
sa `p1r1'
u if round==13 using "${tmp_hfps_eth}/panel/p2_cover.dta", clear
...
sa "${tmp_hfps_eth}/panel/cover.dta", replace
...
sa "${tmp_hfps_eth}/panel/pnl_cover.dta", replace	//	leave all variables in place, to be dropped for the append 
u  "${tmp_hfps_eth}/panel/pnl_cover.dta", clear
...
mer 1:1 household_id round using "${tmp_hfps_eth}/panel/p1_cover.dta"
