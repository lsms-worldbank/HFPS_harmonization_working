...
mer 1:1 household_id round using "${tmp_hfps_eth}/panel/cover.dta", keepus(wgt cs4_sector)
ta round _m	//	one _m=1 in round 6 
keep if _m==3
drop _merge
d using "${tmp_hfps_eth}/panel/demog.dta"
mer 1:1 household_id round using "${tmp_hfps_eth}/panel/demog.dta", keepus(hhsize) assert(2 3) keep(3) nogen
...
sa "${tmp_hfps_eth}/panel/fies.dta", replace
u  "${tmp_hfps_eth}/panel/fies.dta", clear
ta round
