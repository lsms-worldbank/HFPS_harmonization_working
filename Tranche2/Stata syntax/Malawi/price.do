...
ta os_str if mi(unit) & unitstr=="OTHER SPECIFY"
li os_str if mi(unit) & unitstr=="OTHER SPECIFY" & length(os_str)>15
...
isid  y4_hhid round item
sort  y4_hhid round item
sa "${tmp_hfps_mwi}/panel/price.dta", replace 
...
li item itemstr, nol
u  "${tmp_hfps_mwi}/panel/price.dta", clear 
mer m:1 y4_hhid round using "${tmp_hfps_mwi}/panel/cover.dta", assert(2 3) keep(3) nogen keepus(r0_region)
...
recode cf (.=0.001) if _m==1 & inlist(unit,"18","19") & !mi(item)
