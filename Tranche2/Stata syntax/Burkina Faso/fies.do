...
tempfile base
sa      `base'
foreach r of numlist 3/7 9/11 13 15 17/22 {
...
mer 1:1 round hhid using "${tmp_hfps_bfa}/panel/cover.dta", keepus(strate wgt)
ta round _m
...
mer 1:1 round hhid using "${tmp_hfps_bfa}/panel/demog.dta", keepus(hhsize)
ta round _m
bys round (hhid) : egen min_m=min(_merge)
bys round (hhid) : egen max_m=max(_merge)
...
*	merge the downloaded files back in 
	preserve
tempfile out
import delimited using "${tmp_hfps_bfa}/fies/FIES_BFA_out.csv", varn(1) clear
...
loc toappend ""
foreach r of local rounds {
import delimited using "${tmp_hfps_bfa}/fies/FIES_BFA_r`r'_out.csv", varn(1) clear
...
sa "${tmp_hfps_bfa}/panel/fies.dta", replace
