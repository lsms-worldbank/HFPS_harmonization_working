...
d using	"${raw_hfps_uga}/round18/SEC8.dta"
...
la li s8q06a s8q07a s8q08a	//	same as frequency, do not copy labels over we will just use frequency 
for num 6 7 8 : ta qX s8q0Xa,m
g runout_freq = s8q06a if runout==1
g hungry_freq = s8q07a if hungry==1
g whlday_freq = s8q08a if whlday==1
la copy s8q06a frequency
la val *_freq frequency


keep round hhid worried-whlday *_freq

*	get weight and hhsize vars 
d using "${tmp_hfps_uga}/cover.dta"
mer 1:1 round hhid using "${tmp_hfps_uga}/cover.dta", keepus(Rq05 urban /*r0_urban*/ wgt)
ta round _merge
keep if _merge==3
drop _merge

mer 1:1 round hhid using "${tmp_hfps_uga}/demog.dta", keepus(hhsize)
ta round _merge
ta Rq05 if _merge==1,m
su worried-whlday if _merge==1
...
1	43.94	4.70	8.26	2.12
2	31.68	4.49	5.24	1.70
3	20.69	3.73	2.37	1.19
4	20.72	3.95	2.16	1.31
5	19.52	3.81	2.58	1.35
6	22.16	4.24	2.64	1.34
7	46.19	5.17	10.50	2.56
8	50.53	5.20	11.54	2.65
9	48.08	5.14	10.33	2.60
10	53.21	5.69	13.37	3.33
11	38.74	5.55	8.60	2.63
12	33.95	4.85	7.57	2.46
13	41.74	7.01	5.01	2.72
14	39.35	7.87	5.12	4.16
15	33.42	6.62	2.83	2.03
16	48.08	6.70	3.91	2.52
17	44.41	6.97	4.91	2.78
18	40.38	7.15	3.60	2.78
			*/
...
round 18 
	1	All infit inrange(0.7,1.3)
	2	Equating: Skipped is -.50. All are retained for 
		consistency. All others are <=.35
	3	downloaded and saved as FIES_UGA_r18_out.csv
...
*	merge the downloaded files back in 
	preserve
tempfile out
import delimited using "${hfps}/Input datasets/FIES/FIES_UGA_out.csv", varn(1) clear
...
loc toappend ""
foreach r of local rounds {
import delimited using "${hfps}/Input datasets/FIES/FIES_UGA_r`r'_out.csv", varn(1) clear
...
sa "${tmp_hfps_uga}/fies.dta", replace
