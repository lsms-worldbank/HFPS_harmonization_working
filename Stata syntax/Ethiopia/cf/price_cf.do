...
sa "${tmp_hfps_eth}/cf/price_cf.dta", replace 

u  "${tmp_hfps_eth}/cf/price_cf.dta", clear 
la li unit
la save unit using "${do_hfps_eth}/cf/label_unit.do", replace
la li item_cd
la save item_cd using "${do_hfps_eth}/cf/label_item_cd.do", replace

ta unit_cd if item==195	//	purchased injera - no piece 
ta unit_cd if item==196	//	purchased injera bread or biscuits 	->	no piece? 
