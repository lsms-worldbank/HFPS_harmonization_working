...
label_inventory `"${raw_hfps_eth}"', pre(`"r"')	suf(`"_wb_lsms_hfpm_hh_survey_public_microdata.dta"') vardetail retain	/*vardetail varname diagnostic retain*/  
li name-rounds if strpos(varlab,"ncome")>0, sepby(rounds)
label_inventory `"${raw_hfps_eth}"', pre(`"wb_lsms_hfpm_hh_survey_round"')	suf(`"_employment_public.dta"') vardetail retain	/*vardetail varname diagnostic retain*/ 
li name-rounds if strpos(varlab,"ncome")>0, sepby(rounds)
...
*	revenue
numlabel em17_bus_inc em15_bus_inc, add
tab2 round em1?_bus_inc, first m	
numlabel em17_bus_inc em15_bus_inc, remove
egen rvnu = rowfirst(em1?_bus_inc)	
g revenue_lbl_nfe = .
...
sa	"${tmp_hfps_eth}/employment.dta", replace 
u	"${tmp_hfps_eth}/employment.dta", clear
ta sector_nfe refperiod_nfe,m
