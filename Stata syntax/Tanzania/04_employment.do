...
********************************************************************************
********************************************************************************
u hhid s3q*		using	"${raw_hfps_tza}/r9_sect_a_2_3_4_11_12a_14_15_10.dta", clear	//	this file is corrupted in the raw public dataset. There is syntax in the commander do-file to replace the public dta with the csv data, while carrying over the variable and value labels
ta s3q10 s3q01,m
g work_cur = (s3q01==1) if inlist(s3q01,1,2)
g nwork_cur=1-work_cur
g wage_cur = (s3q01==1 & inlist(s3q10,4,5)) if inlist(s3q01,1,2)
...
********************************************************************************
********************************************************************************
// u hhid s3q*		using	"${raw_hfps_tza}/r11_sect_a_2_3_4_11_12a_10.dta", clear
...
********************************************************************************
clear 
append using `r1' `r2' `r3' `r4' `r5' `r6' `r7' `r8' `r9' `r10' `r11', gen(round)
	la drop _append
	la val round .
	ta round 	
	isid hhid round
...
drop revenue_nfe	//	drop the categorical variable in line with the other datasets
...
sa "${tmp_hfps_tza}/employment.dta", replace 
ex
u "${tmp_hfps_tza}/employment.dta", clear
ta sector_cur work_cur,m
ta sector_nfe refperiod_nfe,m
