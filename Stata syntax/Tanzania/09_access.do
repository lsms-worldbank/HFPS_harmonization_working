...
d s5*	using	"${raw_hfps_tza}/r11_sect_5.dta"	//	yes, access
d 		using	"${raw_hfps_tza}/r11_sect_6.dta"	//	availability, no access
d 		using	"${raw_hfps_tza}/r11_sect_7.dta"	//	yes, fuel acces
d 		using	"${raw_hfps_tza}/r11_sect_8.dta"	//	yes, transit access


d s5*	using	"${raw_hfps_tza}/r9_sect_5.dta"	
	*	the r7+ are a distinct structure, can treat them in a batch

label_inventory "${raw_hfps_tza}", pre(`"r"') suf(`"_sect_5.dta"')
label_inventory "${raw_hfps_tza}", pre(`"r"') suf(`"_sect_6.dta"')
label_inventory "${raw_hfps_tza}", pre(`"r"') suf(`"_sect_7.dta"')
label_inventory "${raw_hfps_tza}", pre(`"r"') suf(`"_sect_8.dta"')

label_inventory "${raw_hfps_tza}", pre(`"r"') suf(`"_sect_5.dta"')
qui : label_inventory "${raw_hfps_tza}", pre(`"r"') suf(`"_sect_5.dta"') vallab retain
li lname value label matches rounds if strpos(lname,"t0_")==0,sepby(lname)
label_inventory "${raw_hfps_tza}", pre(`"r"') suf(`"_sect_6.dta"')
...
label_inventory "${raw_hfps_tza}", pre(`"r"') suf(`"_sect_5.dta"') varn
label_inventory "${raw_hfps_tza}", pre(`"r"') suf(`"_sect_6.dta"') varn
label_inventory "${raw_hfps_tza}", pre(`"r"') suf(`"_sect_7.dta"') varn
label_inventory "${raw_hfps_tza}", pre(`"r"') suf(`"_sect_8.dta"') varn
...
sa "${tmp_hfps_tza}/access.dta", replace 
