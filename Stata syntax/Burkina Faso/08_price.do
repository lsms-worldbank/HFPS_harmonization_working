...
d using	"${raw_hfps_bfa}/r23_sec5c_prix_carburant.dta"		
d using	"${raw_hfps_bfa}/r23_sec5p_prix_denrees_alimentaires.dta"		
d using	"${raw_hfps_bfa}/r23_sec5t_prix_transport.dta"		
...
*	update to deal with new units that were added in round 23. We have to reinsert the assumption atht prior rounds relied upon
assert round==23 if inlist(unit,103,104,126,140,141,148)
recode unit (103 140=100)(104 141=143)(126=147)(148=256)
...
mer m:1 hhid round using "${tmp_hfps_bfa}/cover.dta" , assert(2 3) keep(3) nogen keepus(region milieu)
preserve 
u "${hfps}/Input datasets/Burkina Faso/ehcvm_nsu_bfa2021.dta", clear
la li l_Unite
restore 
...
sa "${tmp_hfps_bfa}/price.dta", replace 
