...
d using	"${raw_hfps_bfa}/r22_sec6a_emplrev_general.dta"	
d using	"${raw_hfps_bfa}/r23_sec6a_emplrev_general.dta"	
...
tempfile r21
sa		`r21' 
u	"${raw_hfps_bfa}/r22_sec6a_emplrev_general.dta"		, clear
uselabel s06q04a, clear	//	not labeled in this dataset
tempfile r22
sa		`r22' 
u	"${raw_hfps_bfa}/r23_sec6a_emplrev_general.dta"		, clear
uselabel s06q04a, clear	//	not labeled in this dataset
tempfile r23
sa		`r23' 

u `r1', clear
foreach i of numlist 1/13 15 17 20(1)23 {
mer 1:1 lname value label using `r`i'', gen(_`i')
recode _`i' (2 3=`i')(else=.)
la val _`i' .
}
...
u	"${raw_hfps_bfa}/r23_sec6c_emplrev_nonagr.dta"		, clear
uselabel s06q12, clear	//	not labeled in this dataset
tempfile r23
sa		`r23' 

u `r1', clear
foreach i of numlist 1/4 6 8 9 12 17(2)23 {
mer 1:1 lname value label using `r`i'', gen(_`i')
recode _`i' (2 3=`i')(else=.)
la val _`i' .
}
...
u `r1', clear
foreach i of numlist 1/13 15 17/23 {
mer 1:1 lname value label using `r`i'', gen(_`i')
recode _`i' (2 3=`i')(else=.)
la val _`i' .
}
...
foreach r of numlist 2/13 15 17/23 {
	...
	u `r1', clear
foreach r of numlist 1/13 15 17/23 {
...
preserve
	...
	tempfile r1
	sa    	`r1'
foreach r of numlist 2/4 6 8 9 12 17(2)23 {
	u "${raw_hfps_bfa}/r`r'_sec6c_emplrev_nonagr.dta" , clear
	la dir
	uselabel `r(labels)', clear
	replace label = strtrim(strlower(label))
...
foreach r of numlist 1/4 6 8 9 12 17(2)23 {
	mer 1:1 lname value label using `r`r'', gen(_`r')
	recode _`r' (. 1=.)(2 3=`r')
	la val _`r' .
	la drop _merge
...
#d ; 
foreach r of numlist 2 3 4 6 8 9 12 17(2)23 {;
	u							"${raw_hfps_bfa}/r`r'_sec6a_emplrev_general.dta", clear;
...
clear; append using
	...
	"${raw_hfps_bfa}/r20_sec6a_emplrev_general.dta"		
					`r21'
	"${raw_hfps_bfa}/r22_sec6a_emplrev_general.dta"		
					`r23'
...
replace round=round+1 if round>15
	ta round
	isid hhid round
	ta subsample round	//	rounds 18-23
...
*/
	...
	g nfe_round = (inlist(round,1,2,3,4,6,8,9,12,17,19,21,23))
...
*	revenue
tab2 round s06q12 s06q13 if nfe_round==1, m first
	*	results in round 9 include code 5, which is not documented in qx
		*-> round 17 documents, this is from a split reference period that was implemented,
			*	we will recode 4/5=4 
...
sa	"${tmp_hfps_bfa}/employment.dta", replace
u	"${tmp_hfps_bfa}/employment.dta", clear
